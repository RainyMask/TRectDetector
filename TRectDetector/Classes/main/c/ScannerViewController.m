...
@interface ScannerViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate, TZImagePickerControllerDelegate>
...
@property (nonatomic,strong) CameraSizeBtn *btn_A4;
@property (nonatomic,strong) CameraSizeBtn *btn_16K;
@property (nonatomic,strong) CameraSizeBtn *btn_A6;
@property (nonatomic,strong) CameraSizeMaskView *maskView;//尺寸模式    end ^
/** 1 - A4  2 - 16K  3 - A6*/
...
- (void)_backgroundMode {
    [self stop];
    if ([ScannerCamera sharedInstance].captureDevice) {
        [[ScannerCamera sharedInstance].captureSession stopRunning];
    }
}
...
- (void)start {
    if ([ScannerCamera sharedInstance].captureDevice) {
        ...
        [[ScannerCamera sharedInstance].overLayer removeFromSuperlayer];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [[ScannerCamera sharedInstance].captureSession startRunning];
...
- (UIView *)toolBarView {
    if (!_toolBarView) {
        ...
        [imageBtn setImage:[UIImage imageNamed:@"camera_photo"] forState:UIControlStateNormal];
        ...
        [confirmBtn setImage:[UIImage imageNamed:@"camera_confirm"] forState:UIControlStateNormal];
        [confirmBtn setImage:[UIImage imageNamed:@"camera_confirm_dis"] forState:UIControlStateDisabled];
...
- (void)imageBtnClick:(UIButton *)sender {
    ...
    [KKBaseShow showLoadingInView:self.view];
    [self presentViewController:self.imagePickerController animated:YES completion:^{
        [KKBaseShow hideInView:self.view];
    }];
}
...
- (void)confirmBtnClick:(UIButton *)sender {
    if (self.confirmBlock) {
        ...
        CustomPicBrowserViewController *browseVC = [[CustomPicBrowserViewController alloc] init];
...
- (void)setupCameraView {
    ...
    [[ScannerCamera sharedInstance].dataOutput setSampleBufferDelegate:self queue:_captureQueue];
    //预览实时信息
    [ScannerCamera sharedInstance].previewLayer.frame = self.contentRect;
    [self.view.layer insertSublayer:[ScannerCamera sharedInstance].previewLayer atIndex:0];
...
- (void)addSizeImageView {
    ...
    //选中尺寸后的镂空遮罩
    self.maskView = [[CameraSizeMaskView alloc] initWithFrame:_sizeImageView.bounds rulerTarget:self action:@selector(rulerBtnAction)];
...
- (UIView *)btnView {
    if (!_btnView) {
        ...
        //添加三个尺寸按钮
        self.btn_A4 = [[CameraSizeBtn alloc] initWithLeftTitle:@"210x297mm" rightTitle:@"A4" target:self action:@selector(sizeModelChooseClick:)];
        self.btn_16K = [[CameraSizeBtn alloc] initWithLeftTitle:@"185x260mm" rightTitle:@"16开" target:self action:@selector(sizeModelChooseClick:)];
        self.btn_A6 = [[CameraSizeBtn alloc] initWithLeftTitle:@"105x148mm" rightTitle:@"A6" target:self action:@selector(sizeModelChooseClick:)];
...
- (void)sizeModelChooseClick:(CameraSizeBtn *)sender {
...
- (void)focusAtPoint:(CGPoint)location {
    if (!self.focusIndicator || !self.focusIndicator.superview) {
        ...
        self.focusIndicator.image = [UIImage imageNamed:@"focusIndicator"];
        [self.view addSubview:self.focusIndicator];
    }
    self.focusIndicator.center = location;
    
    [self focusIndicatorAnimate];
    [self cameraFocusAtPoint:location device:[ScannerCamera sharedInstance].captureDevice completionHandler:^{
        [self focusIndicatorAnimate];
    }];
}
...
//        image = [CIImage imageWithCGImage: [self imageByContrastFilter:[self drawImage:image]].CGImage];//滤镜处理  消耗cpu 但是快
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (features.count) {
                ...
                CGFloat area = [IPDFCalculateManager areaOfTheQuadrilateral:features.firstObject];
                ...
                if (_imageDedectionConfidence > 10 && difference > errorValue*3) {//误差15%
                    ...
                    [[ScannerCamera sharedInstance].previewLayer addSublayer:overLayer];
                    [ScannerCamera sharedInstance].overLayer = overLayer;
...
- (void)captureImageWithCompletionHander:(void(^)(UIImage *image))completionHandler {
    ...
    AVCaptureConnection *videoConnection = [[ScannerCamera sharedInstance].stillImageOutput connectionWithMediaType:AVMediaTypeVideo];
    
    [[ScannerCamera sharedInstance].stillImageOutput  captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
