...
- (UIView *)toolBarView {
    if (!_toolBarView) {
        ...
        [closeBtn setImage:[UIImage imageNamed:@"camera_cancel"] forState:UIControlStateNormal];
        [closeBtn addTarget:self action:@selector(closeBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        
        UIButton *switchBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [switchBtn setImage:[UIImage imageNamed:@"switch_normal"] forState:UIControlStateNormal];
        [switchBtn setImage:[UIImage imageNamed:@"switch_sel"] forState:(UIControlStateSelected)];
...
- (UIImageView *)creatVerticeWithAction:(SEL)action {
    ...
    vertice.image = [UIImage imageNamed:@"vertices"];
...
- (void)changeTargetVertice:(UIView *)targetVertice sender:(UIPanGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        if ([targetVertice isEqual:self.topLeftVertice]) {
            temp_TL = location;
            if (![IPDFCalculateManager isQuadrilateralOfPoints:temp_TL topR:temp_TR bottomL:temp_BL bottomR:temp_BR] ||
                [IPDFCalculateManager distanceOfStartPoint:location toEndPoint:temp_BR] < 25) { //最远的顶点不重合
                return;
            }
            ...
            temp_TR = location;
            if (![IPDFCalculateManager isQuadrilateralOfPoints:temp_TL topR:temp_TR bottomL:temp_BL bottomR:temp_BR] ||
                [IPDFCalculateManager distanceOfStartPoint:location toEndPoint:temp_BL] < 25) {
                return;
            }
            ...
            temp_BL = location;
            if (![IPDFCalculateManager isQuadrilateralOfPoints:temp_TL topR:temp_TR bottomL:temp_BL bottomR:temp_BR] ||
                [IPDFCalculateManager distanceOfStartPoint:location toEndPoint:temp_TR] < 25) {
                return;
            }
            ...
            temp_BR = location;
            if (![IPDFCalculateManager isQuadrilateralOfPoints:temp_TL topR:temp_TR bottomL:temp_BL bottomR:temp_BR] ||
                [IPDFCalculateManager distanceOfStartPoint:location toEndPoint:temp_TL] < 25) {
                return;
            }
...
- (void)changeTargetMiddleVertice:(UIView *)targetVertice sender:(UIPanGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateChanged) {
        ...
        //与最远顶点保持距离
        if ([IPDFCalculateManager distanceOfStartPoint:temp_BR toEndPoint:temp_TL] < 25 ||
            [IPDFCalculateManager distanceOfStartPoint:temp_TR toEndPoint:temp_BL] < 25 ) {
            return;
        }
